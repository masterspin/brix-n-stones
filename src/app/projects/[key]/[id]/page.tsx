...
export default function Page({ params }: { params: { key:string, id: string } }) {
    ...
    useEffect(() =>{
      const q = query(collection(db, 'projects'), where("id", "==", params.id.replaceAll("-", " ").replaceAll("%2C",',').replaceAll("%20", " ").replaceAll('%26', '&')))
      const unsubscribe = onSnapshot(q, (QuerySnapshot) => {
        QuerySnapshot.forEach((doc) => {
          let overallRooms = []
          const uploadRef = ref(storage, `${doc.data()['name']}/`);
          listAll(uploadRef).then((response) => {
            for (let i = 0; i < response['prefixes'].length; ++i){
              let urlsArr = []
              let room = {}
              if((response['prefixes'][i]['_location']['path_']).substring((doc.data()['name'] + '/').length) != 'COVER'){
                room['name'] = (response['prefixes'][i]['_location']['path_']).substring((doc.data()['name'] + '/').length)
                const roomRef = ref(storage, `${doc.data()['name']}/${room['name']}/`);
                listAll(roomRef).then((roomResponse) => {
                    roomResponse.items.slice(0,3).forEach((item) => {
    ...
    return (
        ...
        {!isLoading && 
            ...
            <motion.div variants={projectCardsAnimation} initial='initial' animate='animate' className='flex pb-4'>
                <h1 className="text-center mx-auto px-4 text-4xl md:text-5xl font-extrabold leading-none tracking-tight text-gray-900 lg:text-6xl">{items.name}</h1>
            </motion.div>
            <motion.div variants={projectCardsAnimation} initial='initial' animate='animate' className='flex pb-4'>
              <h1 className="text-center mx-auto px-4 md:px-12 lg:px-16 leading-5 text-lg md:text-xl tracking-tight text-gray-600 lg:text-2xl">{items.description}</h1>
            </motion.div>
            </div>
        }
          ...
          {
            (stateRooms).map((proj) =>
              <Link href={'/projects/' + params.key.replaceAll('%2C', ',').replaceAll('%26', '&').replaceAll('%20', ' ') + '/' + items.id.replaceAll(' ', '-') + '/' + proj.name.replaceAll(' ','-')} key={proj.name}>
              <motion.div variants={projectCardsAnimation} initial='initial' animate='animate' className="font-bold w-full text-black hover:text-red-700 rounded overflow-hidden shadow-md py-20 mb-0" style={{backgroundImage: `url(${proj.urls})`, backgroundSize: 'cover', backgroundRepeat:'no-repeat', backgroundColor:'transparent'}}>
                <div className="py-4 my-auto py-auto">
                  <div className="bg-white opacity-80 text-center text-xl px-20 md:px-4 lg:px-16">{proj.name}</div>
                </div>
            </motion.div>
            </Link>
            )
