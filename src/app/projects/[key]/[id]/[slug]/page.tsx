...
export default function Page({ params }: { params: { key:string, id: string, slug: string } }) {
    ...
    useEffect(() =>{
      const q = query(collection(db, 'projects'), where("id", "==", params.id.replaceAll('-', ' ').replaceAll('%2C', ',').replaceAll('%26', '&')))
      const unsubscribe = onSnapshot(q, (QuerySnapshot) => {
        QuerySnapshot.forEach((doc) => {
            let urlsArr = []
          const uploadRef = ref(storage, `${doc.data()['name']}/${params.slug.replaceAll('%20', ' ').replaceAll('%26', '&').replaceAll('-', ' ').replaceAll('%2C', ' ')}/`);
    ...
    return (
        ...
        <div className="flex flex-col items-center justify-center pb-16 pt-14 bg-center bg-cover">
          ...
          {!isLoading && <form className='px-12 md:px-0 lg:px-0 mt-28 md:mt-16 lg:mt-0 xl:mt-0'>
                <motion.button variants={buttonProduct} initial='hidden' animate='show' formAction={ROUTES.PROJECTS + '/' + params.key + '/' + params.id} className="focus:outline-none border-transparent focus:border-transparent focus:ring-0 text-gray-900 font-medium text-base md:text-lg flex items-center gap-2 bg-white hover:bg-pink-100 py-2 px-5 rounded-2xl hover:opacity-80 shadow-lg hover:shadow-2xl">
