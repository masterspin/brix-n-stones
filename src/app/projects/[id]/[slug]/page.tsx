    ...
    useEffect(() =>{
      ...
      const unsubscribe = onSnapshot(q, (QuerySnapshot) => {
        QuerySnapshot.forEach((doc) => {
            let urlsArr = []
          const uploadRef = ref(storage, `${doc.data()['project_id']}.${doc.data()['name']}/${params.slug.replaceAll('%20', ' ')}/`);
    ...
    return (
        ...
        {isLoading && <SkeletonTheme baseColor="#525252" highlightColor="#8f8f8f"><h1><Skeleton /></h1></SkeletonTheme>}
            ...
            <div>
                ...
                <div className='flex pb-4'>
                    <h1 className="mx-auto text-center px-4 text-2xl md:text-3xl font-semibold leading-none tracking-tight text-gray-900 lg:text-4xl dark:text-white">{params.slug.replaceAll('%20', ' ')}</h1>
                </div>
            </div>
